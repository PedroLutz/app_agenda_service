...
export default async (req, res) => {
  try {
    ...
    if (req.method === 'PUT') {
      ...
      if (!id) {
        return res.status(400).json({ error: 'O ID do serviço é obrigatório para a atualização.' });
      }
      ...
      const updatedData = await Servico.findByIdAndUpdate(id, updateFields, { new: true });
